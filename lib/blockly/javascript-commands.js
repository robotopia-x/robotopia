/* global Blockly */
Blockly.JavaScript.marker_placed_event = function (block) {
  const statements = Blockly.JavaScript.statementToCode(block, 'body')

  return `
    robot.onCreateMarker = function (marker) {
      ${statements}
    }\n
  `
}

Blockly.JavaScript.tower_discovered_event = function (block) {
  const statements = Blockly.JavaScript.statementToCode(block, 'body')

  return `
    robot.onDiscoverEntity = function (tower) {
      ${statements}
    }\n
  `
}
