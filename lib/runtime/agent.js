...
class Agent {
  ...
  triggerEvent (name, args) {
    ...
    const engine = this.creatHandlerEngine({ type: 'event', name, args })
  ...
  createHandlerEngine ({ type, name, args }) {
    ...
    const engine = this.currentEngine.fork()
    engine.load(`${this.namespace}.__${type}Handler.apply(${this.namespace}, ${JSON.stringify(args)})`)
  ...
  hasHandler ({ type, name }) {
    ...
    const handlerObject = namespaceScope.get(`__${type}Handler`).next().value.properties[name]
